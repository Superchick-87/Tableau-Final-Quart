...
function drawMatchCellsQuart($pdf, $x, $y, $width_team, $width_score_Quart, $height_cell, $space_x, $space_y, $match_data, $border)
{
    // Définition des dimensions de l'image
    $imageWidth = 10 / 2.5; // Largeur de l'image en pouces
    $imageHeight = 15 / 2.5; // Hauteur de l'image en pouces
    ...
    $pdf->SetFillColor(0, 0, 0, 0);
    $pdf->SetXY($x, $y - 3.5);
    $pdf->Cell($width_team + $width_score_Quart, $height_cell, $match_data[1], $border, 0, 'L', 1, 1, 1, false, '', 'L');
    ...
    $pdf->Cell($width_team, $height_cell, $match_data[2], $border, 0, 'L', 1, 1, 1, false, '', 'L');
    // $pdf->SetXY($x + $width_team + $space_x, $y);
    // $pdf->Cell($width_score_Quart, $height_cell, $match_data[3] . ' | ' . $match_data[4], $border, 0, 'L', 1, 1, 1, false, '', 'M');

    $pdf->SetXY($x + $width_team + $space_x, $y);
    $pdf->Cell($width_score_Quart - 3.5, $height_cell, $match_data[3], $border, 0, 'L', 1, 1, 1, false, '', 'M');

    $pdf->SetXY(1.5 + $x * 2 + $width_team + $space_x, $y);
    $pdf->Cell($width_score_Quart - 3.5, $height_cell, $match_data[4], $border, 0, 'L', 1, 1, 1, false, '', 'M');
    ...
    // #2 Equipes, scores et Ecussons 
    $pdf->SetXY($x, $y + $height_cell + $space_y);
    $pdf->Cell($width_team, $height_cell, $match_data[5], $border, 0, 'L', 1, 1, 1, false, '', 'L');
    // $pdf->SetXY($x + $width_team + $space_x, $y + $height_cell + $space_y);
    // $pdf->Cell($width_score_Quart, $height_cell, $match_data[6] . ' | ' . $match_data[7], $border, 0, 'L', 1, 1, 1, false, '', 'M');

    $pdf->SetXY($x + $width_team + $space_x, $y + $height_cell + $space_y);
    $pdf->Cell($width_score_Quart - 3.5, $height_cell, $match_data[6], $border, 0, 'L', 1, 1, 1, false, '', 'M');

    $pdf->SetXY(1.5 + $x * 2  + $width_team + $space_x, $y + $height_cell + $space_y);
    $pdf->Cell($width_score_Quart - 3.5, $height_cell, $match_data[7], $border, 0, 'L', 1, 1, 1, false, '', 'M');
...
drawMatchCellsQuart($pdf, $x_qf, $y_qf4, $width_team, $width_score_Quart, $height_cell, $space_x, $space_y, $csv[3], $border);
// Utilisation de la fonction pour dessiner Quart de finale 1
drawMatchCellsQuart($pdf, $x_qf, $y_qf1, $width_team, $width_score_Quart, $height_cell, $space_x, $space_y, $csv[0], $border);
// Utilisation de la fonction pour dessiner Quart de finale 2
drawMatchCellsQuart($pdf, $x_qf, $y_qf2, $width_team, $width_score_Quart, $height_cell, $space_x, $space_y, $csv[1], $border);
// Utilisation de la fonction pour dessiner Quart de finale 3
drawMatchCellsQuart($pdf, $x_qf, $y_qf3, $width_team, $width_score_Quart, $height_cell, $space_x, $space_y, $csv[2], $border);
